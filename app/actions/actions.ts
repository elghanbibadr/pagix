...
if (existingProfile) {
  ...
  try {
    const smsResponse = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL}/api/send-verification-sms`,
      {
        method: "POST",
        headers: {
    ...
    if (!smsResponse.ok) {
      ...
      return {
        success: true,
        redirectTo: `/verify-phone?phone=${encodeURIComponent(
          phone
        )}&sms_error=true`,
        warning:
          "Account created but SMS failed to send. Please try resending.",
      };
    }

    console.log("phone", phone);

    console.log("sign up verification code ", verificationCode);
  ...
  return {
    success: true,
    redirectTo: `/verify-phone?phone=${phone}`,
  };
}
...
export async function resendVerificationCode(phone: string) {
  try {
    ...
    const { code: verificationCode,message } = await smsResponse.json();
    ...
    // UPDATE THE USER VERIFCATION CODE
...
export async function sendPasswordVerificationEmail(email: string) {
  ...
  if (error) {
    ...
    return {
      success: false,
      error: "There was an error sending email try later !",
    };
  }
...
export async function sendPasswordVerificationSMS(phone: string) {
  try {
    ...
    if (profile.reset_code_expires_at) {
      const lastCodeTime =
        new Date(profile.reset_code_expires_at).getTime() - 10 * 60 * 1000;
